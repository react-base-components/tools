...
function reportNoConfig() {
  ...
  console.log(chalk.bgRed("!! Unable to setup rb-tools: project's package.json either missing !!"));
  console.log(chalk.bgRed('!! or malformed. Run `npm init` and then reinstall rb-tools.       !!'));
  console.log(
    chalk.bgRed('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
  );
}
...
function reportCompletion() {
  ...
  console.log(chalk.bgGreen('!! rb-tools was successfully installed for the project. !!'));
...
function addConfigHooks(cfg, projectDir) {
  ...
  cfg.scripts = Object.assign(cfg.scripts, {
    build: 'rb-tools run build',
    'gh-pages': 'rb-tools run gh-pages',
    start: 'rb-tools run server',
    pub: 'rb-tools run update-self && rb-tools run pub',
    lint: 'rb-tools run lint',
  });
  ...
  cfg.scripts.prepublish = 'rb-tools run guard';
...
function init() {
  ...
  if (!testMode) {
    ...
    if (!npmArgs || !npmArgs.some(arg => /^rb-tools(@\d+\.\d+.\d+)?$/.test(arg))) {
      return;
    }
  }
  // NOTE: <projectDir>/node_modules/rb-tools/lib
